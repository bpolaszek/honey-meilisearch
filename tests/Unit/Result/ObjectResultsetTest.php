...
it('stores metadata', function () {
    ...
    expect($objects[0])->toBeInstanceOf($city::class)
        ...
        ->and($objects->geo[$objects[0]])->toBe([50.63, 3.06])
        ->and($objects->vectors[$objects[0]])->toBe(['default' => [[-1, 0, 1]]])
        ->and($objects[1])->toBeInstanceOf($city::class)
        ->and($objects[1]->id)->toBe(2)
        ->and($objects[1]->name)->toBe('Paris')
        ->and($objects->geo[$objects[1]])->toBe([48.85, 2.29])
        ->and($objects->vectors[$objects[1]])->toBe(['default' => [[-2, 0, 2]]]);
});
